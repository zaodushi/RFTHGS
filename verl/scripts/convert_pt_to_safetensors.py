...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="The path for your saved model. For megatron, point to the base dir of model, rng, optimizer checkpoints, commonly be `config.default_local_dir/global_step_n`.",
)
...
if args.test:
    ...
    ), f'You must run verl save checkpoint first, with hf_model in checkpoint.contents, and provide the directory here'
...
def convert_megatron_checkpoints_to_hfmodes():
    ...
    def handle_qkv_proj(key, config, tensor, state_dict):
        ...
        if state_dict.get(f'{preffix}.q_proj.{suffix}') is None:
            ...
            state_dict[f'{preffix}.q_proj.{suffix}'] = torch.concat(
                [state_dict[f'{preffix}.q_proj.{suffix}'], q_part], dim=0
            )
        if state_dict.get(f'{preffix}.k_proj.{suffix}') is None:
            state_dict[f'{preffix}.k_proj.{suffix}'] = k_part
        else:
            state_dict[f'{preffix}.k_proj.{suffix}'] = torch.concat(
                [state_dict[f'{preffix}.k_proj.{suffix}'], k_part], dim=0
            )
        if state_dict.get(f'{preffix}.v_proj.{suffix}') is None:
            state_dict[f'{preffix}.v_proj.{suffix}'] = v_part
        else:
            state_dict[f'{preffix}.v_proj.{suffix}'] = torch.concat(
                [state_dict[f'{preffix}.v_proj.{suffix}'], v_part], dim=0
            )
    ...
    def handle_gate_up_proj(key, config, tensor, state_dict):
        ...
        if state_dict.get(f'{preffix}.gate_proj.{suffix}') is None:
            ...
            state_dict[f'{preffix}.gate_proj.{suffix}'] = torch.concat(
                [state_dict[f'{preffix}.gate_proj.{suffix}'], gate_weight_tp], dim=0
            )
        if state_dict.get(f'{preffix}.up_proj.{suffix}') is None:
            state_dict[f'{preffix}.up_proj.{suffix}'] = up_weight_tp
        else:
            state_dict[f'{preffix}.up_proj.{suffix}'] = torch.concat(
                [state_dict[f'{preffix}.up_proj.{suffix}'], up_weight_tp], dim=0
            )
    ...
    if args.test:
        for key, value in state_dict.items():
            ...
            assert (
                value.dtype == ref_state_dict[key].dtype
            ), f'{key} state_dict[key].dtype: {value.dtype} != ref_state_dict[key].dtype: {ref_state_dict[key].dtype}'
